...
import { ActorRegistryService } from '../Services/ActorRegistryService';
import { DefaultDeviceTopologyService } from '../Services/DeviceTopologyService';
import { PlayerRegistryService } from '../Services/PlayerRegistryService';

/** 多人能力包内服务名常量（与历史保持一致，确保 L3 `getService('playerRegistry')` 等无须改动）。 */
export const MULTIPLAYER_SERVICE_NAMES = ['actorRegistry', 'playerRegistry', 'deviceTopology'] as const;
 ...
 */
export function registerMultiplayerCapabilityPack(services: ServiceRegistry, world?: unknown): void {
    if (!services.has('actorRegistry')) {
        services.register('actorRegistry', new ActorRegistryService(), { dependencies: [], lifecycle: ['register', 'dispose'] });
    }
    if (!services.has('playerRegistry')) {
        services.register('playerRegistry', new PlayerRegistryService(), { dependencies: [], lifecycle: ['register', 'dispose'] });
    }
    if (!services.has('deviceTopology')) {
        services.register('deviceTopology', new DefaultDeviceTopologyService(), { dependencies: [], lifecycle: ['register', 'dispose'] });
    }
...
export function isMultiplayerCapabilityPackRegistered(services: ServiceRegistry): boolean {
    return MULTIPLAYER_SERVICE_NAMES.every((name) => services.has(name));
}
