 ...
 */
export class DefaultInterventionSource implements InterventionPort {
    readonly sourceId = 'default';
    private readonly handlers = new Set<(request: InterventionRequest) => void>();
    ...
    dispatchIntervention(request: InterventionRequest): void {
        this.handlers.forEach((handler) => {
            try {
                ...
                console.warn(`[intervention:default] handler threw: ${(error as Error)?.stack ?? error}`);
            }
        });
    }
}
