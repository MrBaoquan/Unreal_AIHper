 ...
 */
export class DefaultSessionStateService implements SessionStateService {
    private role: DeviceRole = 'standalone';
    private roomId: string | undefined;
    private sessionId: string | undefined;
    private worldId: string | undefined;
    private sessionPhase: SessionPhase = 'idle';
    private topologyPhase: TopologyPhase = 'idle';
    private authorityDeviceId: string | undefined;
    private playerCount = 0;
    private lastCommandId: string | undefined;
    private lastCommandStatus: CommandStatus | undefined;
    private pendingMatchSnapshot: PendingMatchSnapshot | undefined;

    private readonly events: EventBus;
    private readonly stateChangedHandlers = new Set<(event: SessionStateChangedEvent) => void>();
    private readonly worldReadyHandlers = new Set<(notification: WorldReadyNotification) => void>();
    ...
    notifyWorldReady(notification: WorldReadyNotification): void {
        ...
        this.worldReadyHandlers.forEach((handler) => {
            try {
                ...
                console.warn(`[session] worldReady handler threw: ${(error as Error)?.stack ?? error}`);
            }
        });
    ...
    private updateField<K extends keyof SessionStateSnapshot>(field: K, value: SessionStateSnapshot[K]): void {
        const previous = this.getSnapshot();
        if (previous[field] === value) {
            return;
        }
        (this as unknown as Record<string, unknown>)[field as string] = value;
        this.emitStateChanged(previous, [field]);
    }

    private emitStateChanged(previous: SessionStateSnapshot, changedFields: readonly (keyof SessionStateSnapshot)[]): void {
        const current = this.getSnapshot();
        const event: SessionStateChangedEvent = { previous, current, changedFields };
        this.stateChangedHandlers.forEach((handler) => {
            try {
                handler(event);
            } catch (error) {
                console.warn(`[session] stateChanged handler threw: ${(error as Error)?.stack ?? error}`);
            }
        });
 ...
 */
export function createSessionStateService(rootContext: FrameworkContext): SessionStateService {
    const events = rootContext.services.get<EventBus>('events');
