...
exports.FrameworkApp = void 0;
const CommandBus_1 = require("./CommandBus");
const DefaultServices_1 = require("./DefaultServices");
const EventBus_1 = require("./EventBus");
const FrameworkContext_1 = require("./FrameworkContext");
const ModuleRegistry_1 = require("./ModuleRegistry");
const ServiceRegistry_1 = require("./ServiceRegistry");
const PlayerInputService_1 = require("../Services/PlayerInputService");
const ResourceService_1 = require("../Services/ResourceService");
const SaveGameService_1 = require("../Services/SaveGameService");
...
class FrameworkApp {
    constructor() {
        ...
        this.rootServices = new ServiceRegistry_1.ServiceRegistry();
        ...
        this.rootModules = new ModuleRegistry_1.ModuleRegistry();
        this.started = false;
    }
    async initialize(options) {
        ...
        globalThis.__uehperFrameworkApp = this;
    ...
    async notifyWorldInitialized(world, nativeInfo) {
    ...
    async notifyNetworkFailure(world, failure) {
    ...
    async createWorldContext(worldInfo) {
        const existing = this.contexts.get(worldInfo.id);
        if (existing) {
            return existing;
        }
        const context = new FrameworkContext_1.FrameworkContext(worldInfo, this.createWorldServices(worldInfo.world), true);
        ...
        try {
            ...
            console.warn(`[uehper] createWorldContext failed for ${context.id}; rolling back. ${error?.stack ?? error}`);
            ...
            if (this.projectApp?.onWorldInitFailed) {
                try {
                    ...
                    console.warn(`[uehper] projectApp.onWorldInitFailed threw; ignoring. ${hookError?.stack ?? hookError}`);
                }
            }
            try {
                ...
                console.warn(`[uehper] createWorldContext rollback destroyWorldContext failed: ${context.id}. ${rollbackError?.stack ?? rollbackError}`);
            }
            throw error;
        }
        return context;
    }
    async destroyWorldContext(worldId) {
        ...
        try {
            ...
            console.warn(`[uehper] World services.disposeAll failed: ${worldId}. ${error?.stack ?? error}`);
        }
    ...
    async shutdown() {
        ...
        try {
            ...
            console.warn(`[uehper] Root services.disposeAll failed. ${error?.stack ?? error}`);
        }
        this.rootServices.clear();
        this.projectApp = undefined;
        this.options = undefined;
        // P0-A: 清理全局引用，避免 shutdown 后残留。
        delete globalThis.__uehperFrameworkApp;
    ...
    registerDefaultServices() {
        ...
        const meta = (name) => (0, DefaultServices_1.findDefaultServiceMeta)('root', name);
        if (!this.rootServices.has('events')) {
            const m = meta('events');
            this.rootServices.register('events', new EventBus_1.EventBus(), { dependencies: m.dependencies, lifecycle: m.lifecycle });
        }
        if (!this.rootServices.has('commands')) {
            const m = meta('commands');
            this.rootServices.register('commands', new CommandBus_1.CommandBus(), { dependencies: m.dependencies, lifecycle: m.lifecycle });
        }
        if (!this.rootServices.has('resources')) {
            const m = meta('resources');
            this.rootServices.register('resources', new ResourceService_1.ResourceService(), { dependencies: m.dependencies, lifecycle: m.lifecycle });
        }
    }
    /** Stage 6.17: 默认服务拓扑的单一数据源；doctor 直接读取避免漂移。 */
    static describeDefaultServices() {
        return {
            root: DefaultServices_1.DEFAULT_SERVICES.root.map((entry) => ({ ...entry, dependencies: [...entry.dependencies], lifecycle: [...entry.lifecycle] })),
            world: DefaultServices_1.DEFAULT_SERVICES.world.map((entry) => ({ ...entry, dependencies: [...entry.dependencies], lifecycle: [...entry.lifecycle] })),
        };
    }
    createRootContext() {
        // root context 不绑定具体 World，services 指向 rootServices，且 dispose 不清空（由 shutdown 统一处理）。
        return new FrameworkContext_1.FrameworkContext({ id: 'root' }, this.rootServices, false);
    }
    createWorldServices(world) {
        const services = this.rootServices.createChild('world');
        const resources = this.rootServices.get('resources');
        // Stage 6.17: lifecycle/dependencies 由 DEFAULT_SERVICES 驱动，与 doctor 保持一致。
        const meta = (name) => (0, DefaultServices_1.findDefaultServiceMeta)('world', name);
        ...
        const rootEvents = this.rootServices.get('events');
        const mEvents = meta('events');
        services.register('events', new EventBus_1.ScopedEventBus(rootEvents), { dependencies: mEvents.dependencies, lifecycle: mEvents.lifecycle });
        const mPlayerInput = meta('playerInput');
        const playerInput = services.register('playerInput', new PlayerInputService_1.PlayerInputService(), { dependencies: mPlayerInput.dependencies, lifecycle: mPlayerInput.lifecycle });
        const mModules = meta('modules');
        services.register('modules', new ModuleRegistry_1.ModuleRegistry(), { dependencies: mModules.dependencies, lifecycle: mModules.lifecycle });
        const mTimers = meta('timers');
        services.register('timers', new TimerService_1.TimerService(), { dependencies: mTimers.dependencies, lifecycle: mTimers.lifecycle });
        ...
        services.register('saveGames', new SaveGameService_1.SaveGameService(world), { dependencies: mSaveGames.dependencies, lifecycle: mSaveGames.lifecycle });
        const mScenes = meta('scenes');
        services.register('scenes', new SceneService_1.SceneService(world), { dependencies: mScenes.dependencies, lifecycle: mScenes.lifecycle });
        const mWorldResources = meta('worldResources');
        const worldResources = services.register('worldResources', new ResourceService_1.ResourceFacade(resources, world), {
            dependencies: mWorldResources.dependencies,
            lifecycle: mWorldResources.lifecycle,
        });
        const mUi = meta('ui');
        services.register('ui', new UIService_1.UIService(worldResources, playerInput), {
            dependencies: mUi.dependencies,
            lifecycle: mUi.lifecycle,
        });
        return services;
    }
    toWorldContextInfo(world, nativeInfo) {
...
//# sourceMappingURL=FrameworkApp.js.map